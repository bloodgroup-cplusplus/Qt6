    ...
    , ui(new Ui::Dialog)
{
    ...
    QState *p = qobject_cast<QState*>(m_states.last()); // could have uesd the state machine children
...
void Dialog::addState(QState *state)
{
    ...
    m_statemachine.addState(state); // Qstates become childern
